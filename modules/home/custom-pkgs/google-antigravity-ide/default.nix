{
  ...
  # FHS environment for running Antigravity IDE
  fhs = buildFHSEnv {
    ...
    runScript = writeShellScript "antigravity-ide-wrapper" ''
      ...
      exec ${antigravity-ide-unwrapped}/lib/antigravity-ide/antigravity-ide "$@"
    '';
  ...
  fhs-package = stdenv.mkDerivation {
    ...
    installPhase = ''
      ...
      mkdir -p $out/bin
      cat <<EOF > $out/bin/antigravity-ide
#!/usr/bin/env bash
exec ${fhs}/bin/antigravity-ide-fhs "\$@"
EOF
...
export CHROME_BIN="${chrome-wrapper}"
export CHROME_PATH="${chrome-wrapper}"
exec $out/lib/antigravity-ide/antigravity-ide "\$@"
EOF
      chmod +x $out/bin/antigravity-ide
