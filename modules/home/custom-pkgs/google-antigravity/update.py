...
def get_latest_version():
    log("Scanning GCS bucket for google-antigravity releases...")
    url = f"{GCS_BUCKET_URL}?prefix=antigravity-hub/"
    candidates = []
    
    pattern = re.compile(r'^antigravity-hub/([0-9.]+-([0-9]+))/linux-x64/Antigravity\.tar\.gz$')
    ...
    while url:
        ...
        if is_truncated_elem is not None and is_truncated_elem.text == 'true':
            ...
            if marker:
                url = f"{GCS_BUCKET_URL}?prefix=antigravity-hub/&marker={urllib.parse.quote(marker)}"
            else:
                url = None
        else:
            url = None
    ...
    if not candidates:
        log("No valid google-antigravity versions found in GCS bucket.")
        sys.exit(1)
...
def update_default_nix(new_version, sri_hash):
    ...
    content = re.sub(r'sha256\s*=\s*"[A-Za-z0-9+/=]+"', f'sha256 = "{sri_hash}"', content) # Fallback match in case it's not starting with "sha256-"
...
def main():
    ...
    if late_parsed <= curr_parsed:
        log("google-antigravity is already up to date.")
        sys.exit(0)
        
    log(f"New version found! Upgrading from {current_ver} to {latest_ver}")
    
    tarball_url = f"{GCS_BUCKET_URL}/antigravity-hub/{latest_ver}/linux-x64/Antigravity.tar.gz"
